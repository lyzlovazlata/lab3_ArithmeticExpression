...
class AVLTree {
    ...
    Node<T>* copyTree(Node<T>* otherRoot) {
        ...
        while (!otkuda.isEmpty()) {
            ...
            if (otk->left) {
                ...
                // соответственно как только опбработали родител€ надо уходить глубже, а из-за стеков мы рассмотрим вообще все
    ...
    void destroyTree(Node<T>* root) {
        ...
        Node<T>* cur = root;
        Node<T>* lastnode = nullptr; // нужн чтобы провер€ть правое поддерево
        ...
        while (!st.isEmpty()||cur) {
            if (cur) {
                ...
                Node<T>* n = st.top();
                // об€хательно смотрим обработан ли правый потомок, потому что иначе потер€ем его!!!!!
...
public:
    ...
    void insert(const T& val) {
        ...
        // вставили теперь балансим (нарушение может быть только у ѕ–≈ƒ ќ¬, поэтому чекаем путь)
        while (!path.isEmpty()) {
            ...
            if (!path.isEmpty()) {
                ...
                //определ€ем какой был потомок и куда прицепить уже «јЅјЋјЌЎ≈Ќ”ё часть
                if (parent->left == node)
    ...
    void erase(const T& val) {
        ...
        if (cur->left && cur->right) {
            ...
            cur->val = minNode->val; // тут € именно присваиваю значение, а не мен€ю указатели
